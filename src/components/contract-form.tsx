...
export function ContractForm() {
  ...
  const { fields: practicalFields, append: appendPractical, remove: removePractical } = useFieldArray({ control: form.control, name: "practicalClassSchedules" });
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <CardContent className="p-4 pt-0 space-y-3">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <FormField control={form.control} name="clientName" render={({ field }) => (
                        <FormItem className="md:col-span-2"><FormLabel className="text-xs">Nombre Completo</FormLabel><FormControl><Input className="h-8 text-sm" placeholder="Juan Pérez" {...field} /></FormControl><FormMessage /></FormItem>
                    )} />
                    <FormField control={form.control} name="studentIdNumber" render={({ field }) => (
                        <FormItem><FormLabel className="text-xs">Cédula / Pasaporte</FormLabel><FormControl><Input className="h-8 text-sm" placeholder="8-000-000" {...field} /></FormControl><FormMessage /></FormItem>
                    )} />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <FormField control={form.control} name="clientEmail" render={({ field }) => (
                        <FormItem><FormLabel className="text-xs">Email</FormLabel><FormControl><Input className="h-8 text-sm" placeholder="correo@ejemplo.com" {...field} /></FormControl><FormMessage /></FormItem>
                    )} />
                    <FormField control={form.control} name="studentPhone1" render={({ field }) => (
                        <FormItem><FormLabel className="text-xs">Teléfono 1</FormLabel><FormControl><Input className="h-8 text-sm" placeholder="6000-0000" {...field} /></FormControl><FormMessage /></FormItem>
                    )} />
                    <FormField control={form.control} name="studentPhone2" render={({ field }) => (
                        <FormItem><FormLabel className="text-xs">Teléfono 2 (Opcional)</FormLabel><FormControl><Input className="h-8 text-sm" placeholder="255-0000" {...field} /></FormControl><FormMessage /></FormItem>
                    )} />
                </div>
                <FormField control={form.control} name="studentAddress" render={({ field }) => (
                    <FormItem><FormLabel className="text-xs">Dirección</FormLabel><FormControl><Input className="h-8 text-sm" placeholder="Calle, Edificio, Casa..." {...field} /></FormControl><FormMessage /></FormItem>
                )} />
            </CardContent>
        </Card>
            ...
            <CardContent className="p-4 pt-0 space-y-3">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <FormField control={form.control} name="courseValue" render={({ field }) => (
                        <FormItem><FormLabel className="text-xs">Valor Total (B/.)</FormLabel><FormControl><Input className="h-8 text-sm" type="number" step="0.01" {...field} /></FormControl></FormItem>
                    )} />
                    <FormField control={form.control} name="downPayment" render={({ field }) => (
                        <FormItem><FormLabel className="text-xs">Abono Inicial (B/.)</FormLabel><FormControl><Input className="h-8 text-sm" type="number" step="0.01" {...field} /></FormControl></FormItem>
                    )} />
                    <FormField control={form.control} name="balance" render={({ field }) => (
                        <FormItem><FormLabel className="text-xs">Saldo</FormLabel><FormControl><Input className="h-8 text-sm bg-muted font-bold text-destructive" type="number" readOnly {...field} /></FormControl></FormItem>
                    )} />
                    <FormField control={form.control} name="paymentDeadline" render={({ field }) => (
                        <FormItem className="flex flex-col">
                            <FormLabel className="text-xs mb-1">Límite Saldo</FormLabel>
                            <Popover>
        ...
        {contractType !== 'Ampliaciones' && (
            <Card className="shadow-sm">
                <CardHeader className="py-3 px-4 flex flex-row items-center justify-between">
                    <CardTitle className="text-base flex items-center gap-2 text-primary">
                        <Clock className="h-4 w-4" /> 4. Clases Prácticas
                    </CardTitle>
                    <Button type="button" variant="outline" size="sm" className="h-7 text-xs" onClick={() => appendPractical({ date: addDays(new Date(), practicalFields.length + 1), time: '8:00am a 10:00am' })}>
                        ...
                        <div key={field.id} className="flex items-center gap-2 p-2 border rounded-md bg-muted/20">
                            ...
                            <FormField control={form.control} name={`practicalClassSchedules.${index}.date`} render={({ field }) => (
                                <FormItem className="flex-1">
                                    <Popover>
                                        <PopoverTrigger asChild>
                                            <FormControl><Button variant="outline" className={cn("h-7 text-xs w-full text-left font-normal", !field.value && "text-muted-foreground")}><CalendarIcon className="mr-1 h-3 w-3" />{field.value ? format(field.value, "dd/MM/yy") : "Fecha"}</Button></FormControl>
                                        </PopoverTrigger>
                                        <PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={field.value} onSelect={field.onChange} initialFocus /></PopoverContent>
                                    </Popover>
                                </FormItem>
                            ...
                            <Button type="button" variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => removePractical(index)} disabled={practicalFields.length <= 1}>
        ...
        <div className="flex justify-end pt-2">
            <Button type="submit" size="lg" className="w-full md:w-auto h-10 px-8 font-bold" disabled={isSubmitting}>
                {isSubmitting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Guardando...</> : 'Generar Contrato'}
            </Button>
        </div>
      </form>
    </Form>
