      ...
      <div className="space-y-3">
        <section className="border border-slate-200 p-3 rounded-sm bg-slate-50/20">
          ...
          <div className="space-y-3 text-justify leading-relaxed">
            <p>EL ESTUDIANTE ha efectuado un <span className="font-bold">ABONO</span> por la suma de <Value>B/. {(details?.downPayment || 0).toFixed(2)}</Value>, quedando un <span className="font-bold">SALDO PENDIENTE</span> de <Value>B/. {(details?.balance || 0).toFixed(2)}</Value>, el cual se compromete a cancelar en su totalidad el día <span className="font-bold underline">{formatDateStr(paymentDeadline)}</span>.</p>
            <p>El <span className="font-bold">VALOR TOTAL</span> del curso es de <Value>B/. {(details?.courseValue || 0).toFixed(2)}</Value>. Para la inscripción, EL ESTUDIANTE deberá abonar el 50% del valor total como reserva de su cupo y horario. El 50% restante deberá cancelarse antes de iniciar la primera clase práctica.</p>
          </div>
        </section>
        ...
        <section className="border border-gray-200 p-3 rounded-sm">
          ...
          <div className="grid grid-cols-2 gap-x-4 text-[8.5pt]">
            ...
            <div className="space-y-1">
                {!isSoloPractica && <div className="text-[8pt] text-gray-600 leading-tight">{(details?.theoreticalClassDates || []).map((d, i) => formatDateStr(d)).join(' | ')}</div>}
            </div>
          </div>
          <div className="mt-3 pt-2 border-t border-gray-200">
            <p className="font-bold mb-1.5 uppercase text-[8pt]">4. Propuesta de Horario Práctico:</p>
            <div className="grid grid-cols-2 gap-x-8 gap-y-1 text-[8.5pt]">
                {(details?.practicalClassSchedules || details?.motoPracticalClassSchedules || []).map((s, index) => (
                    <div key={index} className="flex items-center justify-between border-b border-dotted border-gray-300 pb-0.5"><span className="font-bold">Sesión {index + 1}:</span><span>{formatDateStr(s.date)}</span><span className="font-semibold">{s.time}</span></div>
                ))}
            </div>
          </div>
        </section>
        ...
        <div className="space-y-2 text-[8.2pt] leading-normal text-justify">
            <p><span className="font-bold uppercase">CLÁUSULA TERCERA - INASISTENCIAS:</span> La inasistencia a clase práctica sin notificación previa de 24h resultará en pérdida de la sesión. Reprogramación sujeta a disponibilidad médica o pago de recargo de <span className="font-black">$20.00</span>.</p>
            <p><span className="font-bold uppercase">CLÁUSULA SEXTA - VESTIMENTA:</span> Se prohíbe el uso de escotes, minifaldas, camisillas, pantalones cortos, leggins o chancletas. El incumplimiento implica pérdida de clase.</p>
            <p><span className="font-bold uppercase">CLÁUSULA NOVENA - CANCELACIÓN:</span> No habrá devolución de dinero bajo ninguna circunstancia en caso de cancelación por parte del cliente.</p>
            <p><span className="font-bold uppercase">CLÁUSULA UNDÉCIMA - VIGENCIA:</span> El contrato tiene una vigencia de tres (3) meses desde su firma.</p>
        </div>
        ...
        <section className="mt-4 pt-4 border-t-2 border-black">
            ...
            <p className="text-center italic text-[8.5pt] mb-8">Suscrito el día <Value>{!isNaN(creationDate.getTime()) ? format(creationDate, 'd', { locale: es }) : '---'}</Value> de <Value>{!isNaN(creationDate.getTime()) ? format(creationDate, 'MMMM', { locale: es }) : '---'}</Value> de <Value>{!isNaN(creationDate.getTime()) ? format(creationDate, 'yyyy', { locale: es }) : '---'}</Value>.</p>
            <div className="flex justify-between px-16 pt-8">
                <div className="text-center w-[220px]"><div className="border-t border-black mb-1"></div><p className="font-bold uppercase text-[7.5pt]">Por la Empresa</p></div>
                <div className="text-center w-[220px]"><div className="border-t border-black mb-1"></div><p className="font-bold uppercase text-[7.5pt]">El Cliente</p></div>
            </div>
        </section>
      </div>
      <div className="mt-10 text-center text-[6.5pt] text-gray-400 uppercase">Documento interno • Confeccionado por: {contract.createdBy || 'Sistema'} • {format(new Date(), 'PPpp', { locale: es })}</div>
    </div>
  );
}
