...
export default function DynamicEnrollPage() {
  ...
  const handleNextStep = async () => {
    if (step === 1) {
      ...
      // Validar cupos en tiempo real antes de avanzar al pago
      ...
      // Avanzar al paso 4 (Pago) sin escribir en la base de datos para no bloquear cupos si abandona
      setStep(4);
      window.scrollTo({ top: 0, behavior: 'smooth' });
    }
  };
  ...
  // Submit final en Paso 4: Solo aquí se crea el contrato y se reserva formalmente el cupo tras confirmar el pago
  const onSubmit = async (data: z.infer<typeof enrollmentSchema>) => {
    ...
    try {
      ...
      // Crear nuevo contrato en Firestore de manera atómica con Folio Oficial
      ...
      await runTransaction(db, async (transaction) => {
        ...
        const contractData = {
          ...
          createdBy: 'Inscripción Web Oficial',
          isOnline: true,
          source: 'online',
          autoMotoDetails: {
      ...
      // Notificación al WhatsApp del Asesor con el comprobante de pago
      ...
      toast({ title: "Error de Registro", description: "No se pudo registrar la matrícula. Inténtalo nuevamente.", variant: "destructive" });
    } finally {
