...
function CertificateFrontAmpliacion({ certificate }: { certificate: Certificate }) {
    ...
    return (
        <div className="w-full min-h-screen flex items-center justify-center bg-white print:block print:h-auto break-after-page">
            {/* Margen asimétrico: pl-24 para aumentar izquierdo, pr-4 para reducir derecho */}
            <div className="w-[10.5in] h-[8in] bg-white text-black font-serif mx-auto print:m-0 relative flex items-center justify-center pl-24 pr-4 py-8">
                <div className="w-full h-full border-[4px] border-black flex flex-col p-8 relative overflow-hidden">
                    ...
                    <header className="flex w-full flex-col items-center justify-center relative pt-2">
                        ...
                        <h1 className="text-[21pt] font-black tracking-[0.15em] leading-none mb-1 text-black">FREEWAY</h1>
                        <p className="text-[9.75pt] tracking-[0.4em] font-semibold text-black uppercase">Escuela de Manejo</p>
                        <p className="text-[7.5pt] italic mt-1">Casa Matriz Chorrera</p>

                        <div className="absolute top-0 right-0 text-center">
                            <p className="text-[16.5pt] font-black mb-1">{getHighestLicenseType(certificate.licenseType)}</p>
                            <p className="text-[6.75pt] font-bold border-t border-black pt-1">{folioNum} / {folioYear}</p>
                        </div>
                    </header>
                    ...
                    <main className="flex-grow flex flex-col items-center justify-center text-center px-8 mt-1">
                        ...
                        <div className="mb-4 w-full">
                            <p className="font-black text-[15pt] tracking-tighter leading-none mb-2 uppercase">
                                {certificate.clientName}
                            </p>
                            <p className="font-bold text-[12.75pt] tracking-widest">{certificate.idType || 'C.I.P.'} &nbsp; {certificate.cip}</p>
                        </div>

                        <div className="text-[9.75pt] leading-relaxed max-w-3xl mx-auto py-3 px-6 border border-dashed border-black/20 rounded-xl">
                            <p>
                        ...
                        <div className="text-[6.75pt] mx-auto mt-6 max-w-xl font-bold opacity-80">
                            <p>
                                Reconocida por la Autoridad del Tránsito y Transporte Terrestre, Resuelto N°380 (04 de diciembre de 2000) Resolución AL-325
                            </p>
                        </div>
                        
                        <div className="text-center mt-4 font-bold text-[5.6pt] italic">
...
function CertificateBackAmpliacion({ certificate }: { certificate: Certificate }) {
    ...
    return (
        <div className="w-full min-h-screen flex items-center justify-center bg-white print:block print:h-auto">
            {/* Margen asimétrico también en el reverso */}
            <div className="w-[10.5in] h-[8in] p-12 pl-24 pr-4 bg-white text-black font-sans text-[10.5pt] flex flex-col justify-start mx-auto print:m-0 flex items-center justify-center">
                <div className="space-y-8 pt-8 w-full">
                    ...
                    <p><span className="font-bold uppercase tracking-widest text-[9pt]">TIPO DE LICENCIAS:</span> <span className="font-black">{certificate.licenseType}</span></p>
                    ...
                    <div className="pt-8 space-y-8 mt-12">
                        <div className="flex gap-x-12">
                            <div className="flex flex-col gap-1 flex-1">
                                ...
                                <span className="font-black text-[12.75pt]">{certificate.firstName?.toUpperCase() || ''}</span>
                            </div>
                            <div className="flex flex-col gap-1 flex-1">
                                <span className="text-[6pt] font-black uppercase text-slate-400">Segundo Nombre</span>
                                <span className="font-black text-[12.75pt]">{certificate.middleName?.toUpperCase() || ''}</span>
                            </div>
                        </div>
                        <div className="flex gap-x-12">
                            <div className="flex flex-col gap-1 flex-1">
                                <span className="text-[6pt] font-black uppercase text-slate-400">Primer Apellido</span>
                                <span className="font-black text-[12.75pt]">{certificate.lastName?.toUpperCase() || ''}</span>
                            </div>
                            <div className="flex flex-col gap-1 flex-1">
                                <span className="text-[6pt] font-black uppercase text-slate-400">Segundo Apellido</span>
                                <span className="font-black text-[12.75pt]">{certificate.secondLastName?.toUpperCase() || ''}</span>
                            </div>
                        </div>
                    </div>
                </div>
