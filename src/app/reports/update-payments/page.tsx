...
export default function UpdatePaymentsPage() {
  ...
  const filteredPayments = useMemo(() => {
    ...
    return payments.filter((p) => String(p.updateFolio).includes(search) || p.clientName?.toLowerCase().includes(search));
  }, [payments, searchTerm]);
        ...
        <Table className="border rounded-lg">
          ...
          <TableBody>
            {filteredPayments.map(p => (
              <TableRow key={p.id}>
                <TableCell>{String(p.updateFolio).padStart(6, '0')}</TableCell>
