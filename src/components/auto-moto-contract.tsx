      ...
      <div className="space-y-2">
        <section>
          ...
          <p className="text-justify leading-snug">
            El estudiante ha efectuado un <span className="font-bold">ABONO</span> por la suma de <Value>B/. {(details?.downPayment || 0).toFixed(2)}</Value>, quedando un <span className="font-bold">SALDO PENDIENTE</span> de <Value>B/. {(details?.balance || 0).toFixed(2)}</Value>, el cual se compromete a cancelar en su totalidad el día <span className="font-bold underline">{formatDateStr(paymentDeadline)}</span>. El valor total del curso es de <Value>B/. {(details?.courseValue || 0).toFixed(2)}</Value>. Para la inscripción, EL ESTUDIANTE deberá abonar el 50% del valor total como reserva de su cupo y horario. El 50% restante deberá cancelarse antes de iniciar la primera clase práctica. En caso de incumplimiento en los pagos, EL ESTUDIANTE no podrá continuar el curso.
          </p>
        </section>
