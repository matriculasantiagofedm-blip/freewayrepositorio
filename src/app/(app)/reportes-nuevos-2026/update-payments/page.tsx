...
import { Loader2, RefreshCw, CalendarIcon, ChevronLeft } from 'lucide-react';
...
export default function UpdatePaymentsReport() {
  const db = useDb();
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black uppercase tracking-tight text-purple-600">Reporte de Actualizaciones</h1>
            <p className="text-xs font-bold text-muted-foreground uppercase">Ingresos por corrección o vigencia de certificados.</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="md:col-span-2 shadow-sm">
          <CardHeader className="py-3 px-6 bg-slate-50/50 border-b">
            <CardTitle className="text-[10px] font-black uppercase tracking-widest text-slate-500">Listado de Actualizaciones Pagadas</CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <div className="p-12 text-center text-xs font-bold text-slate-400 italic">No hay registros de actualizaciones hoy.</div>
            )}
          </CardContent>
        </Card>

        <Card className="bg-purple-600 text-white border-none shadow-xl h-fit">
          <CardHeader>
            <CardTitle className="text-xs font-black uppercase tracking-widest opacity-70">Total Actualizaciones</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-4xl font-black">B/. {totalAmount.toFixed(2)}</p>
            <p className="text-[9px] font-bold uppercase mt-2 opacity-60">Ingresos brutos por trámites de duplicados o actualizaciones.</p>
          </CardContent>
        </Card>
      </div>
    </div>
