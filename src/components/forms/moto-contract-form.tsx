...
import { UserCircle, Settings2, Save, Clock, Loader2, CalendarIcon, DollarSign } from 'lucide-react';
...
const motoPackages = [
    { id: 'basico', label: 'Curso Moto Básico (8hrz)', price: 133.00, hours: 8 },
    { id: 'plus', label: 'Curso Moto Plus (10hrz)', price: 155.00, hours: 10 },
    { id: 'premium', label: 'Curso Moto Premium (12hrz)', price: 180.00, hours: 12 },
];
...
const schema = z.object({
  ...
  vehicleTransmission: z.string().default('Moto'),
  licenseCategory: z.string().default('A, B'),
...
export function MotoContractForm({ initialContract }: { initialContract?: Contract }) {
  const db = useDb();
  const { user } = useUser();
  ...
  const form = useForm<z.infer<typeof schema>>({
    ...
    defaultValues: initialContract ? {
      ...
      licenseCategory: initialContract.autoMotoDetails?.licenseCategory || 'A, B',
      ...
      vehicleTransmission: 'Moto', licenseCategory: 'A, B', practicalClassSchedules: [],
    },
  });
  ...
  const onSubmit = async (values: z.infer<typeof schema>) => {
    ...
    try {
      await runTransaction(db, async (transaction) => {
        ...
        const finalData = {
            id: contractId, folioNumber: folio, title: 'Curso Moto', clientName: values.clientName, clientEmail: values.clientEmail,
            clientId: values.studentIdNumber, type: 'Curso Moto', status: initialContract?.status || 'active',
            ...
            autoMotoDetails: {
                ...
                vehicleTransmission: 'Moto', licenseCategory: values.licenseCategory,
      ...
      toast({ title: 'Éxito', description: 'Contrato de moto guardado.' });
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 max-w-5xl mx-auto pb-20">
        ...
        <Card className="shadow-md">
            <CardHeader className="bg-slate-50/50 border-b py-3 px-6"><div className="flex items-center gap-2"><Settings2 className="h-5 w-5 text-orange-600" /><CardTitle className="text-sm font-bold uppercase tracking-tight">Configuración del Curso Moto</CardTitle></div></CardHeader>
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6">
                <FormField control={form.control} name="coursePlan" render={({ field }) => (<FormItem><FormLabel className="text-xs uppercase font-bold text-muted-foreground">Plan de Curso</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger className="h-10"><SelectValue placeholder="Elegir..." /></SelectTrigger></FormControl><SelectContent>{motoPackages.map(p => <SelectItem key={p.id} value={p.id}>{p.label}</SelectItem>)}</SelectContent></Select></FormItem>)} />
                <FormField control={form.control} name="licenseCategory" render={({ field }) => (<FormItem><FormLabel className="text-xs uppercase font-bold text-muted-foreground">Categoría (A, B)</FormLabel><FormControl><Input {...field} className="h-10 font-bold" /></FormControl></FormItem>)} />
            </CardContent>
        </Card>

        <Card className="shadow-md">
            <CardHeader className="bg-slate-50/50 border-b py-3 px-6"><div className="flex items-center gap-2"><Clock className="h-5 w-5 text-orange-600" /><CardTitle className="text-sm font-bold uppercase tracking-tight">Agenda Práctica Moto</CardTitle></div></CardHeader>
                ...
                {fields.map((field, i) => {
                    ...
                    return (
                        <div key={field.id} className={cn("grid grid-cols-4 gap-3 p-3 border rounded-lg bg-white relative", (isBusy || isFull || isSun || hol) && "border-amber-500 bg-amber-50/30")}>
                            ...
                            <FormField control={form.control} name={`practicalClassSchedules.${i}.date`} render={({ field: f }) => (<FormItem><Popover modal={true}><PopoverTrigger asChild><FormControl><Button variant="outline" className="w-full h-9 text-xs"><CalendarIcon className="mr-2 h-3 w-3" /> {f.value ? format(toDate(f.value), "dd/MM/yy") : "Fecha"}</Button></FormControl></PopoverTrigger><PopoverContent className="w-auto p-0"><Calendar mode="single" selected={f.value || undefined} onSelect={f.onChange} initialFocus /></PopoverContent></Popover></FormItem>)} />
                            <FormField control={form.control} name={`practicalClassSchedules.${i}.time`} render={({ field: f }) => (<FormItem><Select onValueChange={f.onChange} value={f.value}><FormControl><SelectTrigger className="h-9 text-xs"><SelectValue placeholder="Turno" /></SelectTrigger></FormControl><SelectContent>{practicalTimeSlots.map(tSlot => <SelectItem key={tSlot} value={tSlot} className="text-xs">{tSlot}</SelectItem>)}</SelectContent></Select></FormItem>)} />
                            <FormField control={form.control} name={`practicalClassSchedules.${i}.vehicle`} render={({ field: f }) => (<FormItem><Select onValueChange={f.onChange} value={f.value}><FormControl><SelectTrigger className="h-9 text-xs"><SelectValue placeholder="Vehículo" /></SelectTrigger></FormControl><SelectContent>{motoVehicles.map(vName => <SelectItem key={vName} value={vName} className="text-xs">{vName}</SelectItem>)}</SelectContent></Select></FormItem>)} />
        ...
        <Card className="shadow-md">
            <CardHeader className="bg-slate-50/50 border-b py-3 px-6"><div className="flex items-center gap-2"><DollarSign className="h-5 w-5 text-orange-600" /><CardTitle className="text-sm font-bold uppercase tracking-tight">Información de Pago</CardTitle></div></CardHeader>
        ...
        <div className="flex justify-end gap-4">
            ...
            <Button type="submit" size="lg" className="px-12 h-14 text-lg font-bold shadow-xl bg-orange-600 hover:bg-orange-700" disabled={isSaving}>
                {isSaving ? <Loader2 className="animate-spin mr-2" /> : <Save className="mr-2" />} 
                {initialContract ? 'Actualizar Contrato' : 'Generar Contrato Moto'}
            </Button>
        </div>
      </form>
    </Form>
