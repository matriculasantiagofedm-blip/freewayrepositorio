...
import { UserCircle, Settings2, Save, DollarSign, Clock, Loader2 } from 'lucide-react';
...
export function DeluxeContractForm({ initialContract }: { initialContract?: Contract }) {
  ...
  const onSubmit = async (values: z.infer<typeof schema>) => {
    ...
    try {
      ...
      toast({ title: 'Éxito', description: 'Contrato Deluxe generado.' });
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 max-w-5xl mx-auto pb-20">
        ...
        <Card className="shadow-md">
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-6">
                ...
                <FormField control={form.control} name="startDate" render={({ field }) => (<FormItem><FormLabel className="text-xs uppercase font-bold text-muted-foreground">Fecha de Inicio (Semana 1)</FormLabel><Popover modal={true}><PopoverTrigger asChild><FormControl><Button variant="outline" className="w-full h-10 text-xs">{field.value ? format(field.value, "dd/MM/yyyy") : "Elegir"}</Button></FormControl></PopoverTrigger><PopoverContent className="w-auto p-0"><Calendar mode="single" selected={field.value} onSelect={field.onChange} initialFocus /></PopoverContent></Popover></FormItem>)} />
            </CardContent>
        </Card>
