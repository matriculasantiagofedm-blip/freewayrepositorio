...
export default function TheoreticalSchedulePage() {
  ...
  // Unificar alumnos de la semana (Sin duplicados por contrato)
  const weeklyCitations = useMemo(() => {
    ...
    contracts?.forEach(c => {
      ...
      // Si el alumno tiene al menos una cita esta semana, añadirlo una sola vez
      ...
      <Card id="theory-report-print" className="shadow-xl border-none m-2 overflow-hidden">
        <CardHeader className="bg-slate-900 text-white py-6">
            <div className="flex justify-between items-center">
                <div>
                    <CardTitle className="text-xl font-black uppercase tracking-widest">Citaciones Teóricas de la Semana</CardTitle>
        ...
        <CardContent className="p-0">
          {isLoading ? (
              ...
              <TableBody>
                {weeklyCitations.map((item) => (
                  <TableRow key={item.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <TableCell>
                      <Badge variant="outline" className="text-[9px] font-black uppercase border-slate-200 bg-white text-slate-500 py-0 h-5">
                          {item.type}
                      </Badge>
                    </TableCell>
                    <TableCell>
            ...
            <div className="p-32 text-center flex flex-col items-center gap-4 opacity-30">
                ...
                <p className="font-black uppercase text-xs tracking-widest text-slate-500">No hay citaciones teóricas registradas para esta semana</p>
            </div>
          )}
        </CardContent>
      </Card>
