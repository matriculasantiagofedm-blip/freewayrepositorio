...
export function DeluxePremiumContractTemplatePreview({ clientName, clientEmail, deluxeDetails }: DeluxePremiumContractTemplatePreviewProps) {
  ...
  const formatInstallmentDate = (dateString?: string | Date) => {
    if (!dateString) return <Line className="min-w-20" />;
    const date = toDate(dateString);
    if (isNaN(date.getTime())) return <Line className="min-w-20" />;
    try {
  ...
  return (
    <Card className="p-6 print:shadow-none print:border-none print:p-0 font-serif text-xs">
      <CardContent className="p-0 space-y-1 relative">
        <h2 className="text-center font-bold text-sm mb-2">CONTRATO DE SERVICIOS EDUCATIVOS</h2>
        ...
        <p><Value>Clases teóricas: Miércoles, de 6:00 p.m. a 8:00 p.m.</Value></p>
