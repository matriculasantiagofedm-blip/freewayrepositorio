    ...
    <Card className="p-6 print:shadow-none print:border-none print:p-0 font-serif text-xs">
      <CardContent className="p-0 space-y-2 relative">
        ...
        <p className="text-center !mt-4">
            En fe de lo cual, se suscribe el presente contrato en la ciudad de Panamá, República de panamá, a los <Value>{format(creationDate, 'd')}</Value> días del mes de <Value>{format(creationDate, 'LLLL', { locale: es })}</Value>, de <Value>{format(creationDate, 'yyyy')}</Value>, a las <Value>{format(creationDate, 'p', { locale: es })}</Value>.
        </p>
        ...
        <div className="flex justify-around pt-12">
            ...
            <div className="text-center flex flex-col items-center">
                <span className="border-b border-black w-48 block"><Value className="text-black">{contract.clientName}</Value></span>
