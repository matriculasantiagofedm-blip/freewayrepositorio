...
import { Loader2, Printer, CalendarIcon, ChevronLeft, TrendingUp } from 'lucide-react';
    ...
    <div className="flex flex-col gap-6 pb-20">
        <div className="flex items-center justify-between print:hidden">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="font-headline text-3xl font-bold uppercase">Reporte de Cancelaciones</h1>
                    <p className="text-muted-foreground text-xs">Historial mensual de pagos de saldos.</p>
                </div>
            </div>
        ...
        <Card className="bg-blue-600 text-white border-none shadow-lg">
            <CardContent className="p-6 flex items-center justify-between">
                <div>
                    <p className="text-[10px] font-black uppercase opacity-60 tracking-widest">Total Recaudado en {format(selectedDate, "MMMM", { locale: es })}</p>
        ...
        <Card className="shadow-none border-slate-200">
            <CardHeader className="bg-slate-50 border-b py-2"><CardTitle className="text-xs font-black uppercase">Desglose de Pagos de Saldos</CardTitle></CardHeader>
            <CardContent className="p-0">
                <Table>
                    <TableHeader><TableRow className="bg-slate-100">
                        ...
                        <TableHead className="font-black text-[9px] uppercase">Contrato</TableHead>
                    ...
                    <TableBody>
                        {payments.map((p) => (
                            <TableRow key={p.id} className="text-[10px] h-8">
                                <TableCell className="font-black text-blue-600">{String(p.cancellationFolio || '').padStart(6, '0')}</TableCell>
                                <TableCell>{format(toDate(p.paymentDate), "dd/MM/yy")}</TableCell>
                                <TableCell className="uppercase font-bold">{p.clientName}</TableCell>
                                <TableCell className="font-bold opacity-60">Folio {String(p.contractFolio || '').padStart(6, '0')}</TableCell>
                        ...
                        {payments.length === 0 && <TableRow><TableCell colSpan={6} className="h-24 text-center text-slate-400 italic">Sin resultados.</TableCell></TableRow>}
                    </TableBody>
                </Table>
            </CardContent>
        </Card>
